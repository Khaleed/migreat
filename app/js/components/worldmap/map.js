...
let countriesJSON = topojson.feature(worldMap, worldMap.objects.countries)
  .features;
...
countries
  ...
  .datum(
    topojson.mesh(worldMap, worldMap.objects.countries, (a, b) => {
      // datums set and get bound data for each selected element
      return a !== b;
    })
  )
  .attr("class", "boundary")
...
let zoom = d3.behavior.zoom().on("zoom", function() {
  ...
  countries.selectAll("path").attr("d", path.projection(projection));
});
countries.call(zoom);
